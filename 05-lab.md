...
``` r
met_stations <- met[, .(wind.sp = mean(wind.sp, na.rm = TRUE), atm.press = mean(atm.press, na.rm = TRUE), temp = mean(temp, na.rm = TRUE)), by = .(USAFID, STATE)]

met_stations[, temp50 := quantile(temp, probs = .5, na.rm = TRUE)]
met_stations[, atm50 := quantile(atm.press, probs = .5, na.rm = TRUE)]
met_stations[, windsp50 := quantile(wind.sp, probs = .5, na.rm = TRUE)]

met_stations[which.min(abs(temp - temp50))]
```

    ##    USAFID STATE  wind.sp atm.press     temp   temp50    atm50 windsp50
    ## 1: 720458    KY 1.209682       NaN 23.68173 23.68406 1014.691 2.461838

``` r
met_stations[which.min(abs(atm.press - atm50))]
```
...
met_stations[, atm50s := quantile(atm.press, probs = .5, na.rm = TRUE), by = STATE]
...
``` r
met_stations[, tempdif := which.min(abs(atm.press - atm50s)), by = STATE]
met_stations[recordid == tempdif, .(USAFID, atm.press, atm50s, STATE)]
```

    ##     USAFID atm.press   atm50s STATE
    ##  1: 720254       NaN       NA    WA
    ##  2: 720855       NaN       NA    ND
