      ...
      select(word1, word2) %>%
        filter(word1 == "patient") %>%
          count(word2, sort = TRUE) %>%
            top_n(20, n) %>%
              ggplot(aes(x = n, y = fct_reorder(word2, n))) + geom_col()
      ...
      select(word1, word2) %>%
        filter(word2 == "patient") %>%
          count(word1, sort = TRUE) %>%
            top_n(20, n) %>%
              ggplot(aes(x = n, y = fct_reorder(word1, n))) + geom_col()
```
---
...
- "the operating room" and "tolerate the procedure" ngrams are quite interesting and suggest the transcription records are likely from a surgical department.
